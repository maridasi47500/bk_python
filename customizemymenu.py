        ...
        if unefois == False:
            for myvalues in dataparams.values():
	    	    ...
	    	    if all( item[0] in ["side","drink","burger","burgerid"] for item in othervalues):
                            ...
                            code+=self.display_collection(sql,values,"/custom/_"+value,erreurmsg,tablename)
                            tablename="burgers"
                            sql="select * from burgers where burgercat_id = 4 limit 1"
                            values=()
                            value="sideavecdrink"
                            code+=self.display_collection(sql,values,"/custom/_"+value,erreurmsg,tablename).replace("mataille",query_components["side"][0])
                            tablename="burgers"
                            sql="select * from burgers where burgercat_id = 5 limit 1"
                            values=()
                            value="drinkavecside"
                            code+=self.display_collection(sql,values,"/custom/_"+value,erreurmsg,tablename).replace("mataille",query_components["drink"][0])
                            break
	    	    if all( dic in ["burger","burgerid"] for dic in query_components.keys()) & any(query_components[dic][-1] in ["small","medium","large","value"] for dic in query_components):
                            ...
                            code+=self.display_collection(sql,values,"/custom/_"+value,erreurmsg,tablename).replace("mataille",mydic[query_components["burger"][-1]])
                            break
	    	    if unefois == True:
	    	        break		
                        ...
                        try:
                            ...
                            for param1, value in othervalues:
                                ...
                                if value in ["burger","sandwich"]:
                                    ...
                                    values=("5")
                                    code+=self.display_collection(sql,values,"custom/_"+param1,erreurmsg,tablename).replace("mataille",value)
                                    #replace size with value
